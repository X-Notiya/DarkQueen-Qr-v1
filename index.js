...
const {  state, saveCreds } =await useMultiFileAuthState(`./sessions`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`මෙතෙන්ට ඔයාගෙ නම්බරෙ දෙන්න: `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')

         // Ask again when entering the wrong number
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("❄ꜱᴛᴀʀᴛ ᴡɪᴛʜ ᴄᴏᴜɴᴛʀʏ ᴄᴏᴅᴇ ᴏꜰ ʏᴏᴜʀ ᴡʜᴀᴛꜱᴀᴘᴘ ɴᴜᴍʙᴇʀ❄(ᴇxᴀᴍᴘʟᴇ)= +94715491788 ")))
    ...
    XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
          ...
          let c = output.data.split('/')[3]
           await XeonBotInc.sendMessage(XeonBotInc.user.id, {text: '𝐃𝐚𝐫𝐤_𝐐𝐮𝐞𝐞𝐧_𝐌𝐃;;;'+c});	
